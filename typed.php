...
if (!class_exists('Zon_Typed')) :
    ...
    class Zon_Typed {
        ...
        public function typed_perview_meta_box_callback($post) {
            ...
            $strings = esc_html(trim($strings, ';#$#;'));
            $font_size = esc_html(get_post_meta($post_id, 'font_size', true));
            $font_type = esc_html(get_post_meta($post_id, 'font_type', true));
            $color = esc_html(get_post_meta($post_id, 'font_color', true));
            $font_weight = esc_html(get_post_meta($post_id, 'font_weight', true));
            $anchor_color = esc_html(get_post_meta($post_id, 'anchor_color', true));
            $cursor_color = esc_html(get_post_meta($post_id, 'cursor_color', true));
            ?>
            <style>
            ...
            <table class="widefat" style="border: 0px;">
                <tbody>
                    <tr>
                        ...
                        <td class="typed_strings_container">
                            ...
                            if (count($typed_strings) > 0):
                                foreach ($typed_strings as $string):
                                    ?>
                                    <p><input type="text" name="typed_strings[]" value="<?php echo esc_attr(str_replace('"', "'", $string)); ?>"/> <a href="javascript:void(0)" class="add_typed_string button-secondary">+</a> <?php if ($index > 0): ?><a href="javascript:void(0)" class="remove_typed_string button-secondary">-</a><?php endif; ?></p>
                                    <?php
                                    $index++;
                                endforeach;
                            else:
                    ...
                    <tr>
                        <td>Type Speed</td>
                        <td>
                            <input type="text" value="<?php echo esc_attr(get_post_meta($post_id, 'type_speed', true)) ? esc_attr(get_post_meta($post_id, 'type_speed', true)) : 0; ?>" name="type_speed"/>
                            <br><span class="description">type speed in milliseconds </span>
                        </td>
                    </tr>
                    <tr>
                        <td>Start Delay</td>
                        <td>
                            <input type="text" value="<?php echo esc_attr(get_post_meta($post_id, 'start_delay', true)) ? esc_attr(get_post_meta($post_id, 'start_delay', true)) : 0; ?>" name="start_delay"/>
                            <br><span class="description">time before typing starts in milliseconds </span>
                        </td>
                    </tr>
                    <tr>
                        <td>Back Speed</td>
                        <td>
                            <input type="text" value="<?php echo esc_attr(get_post_meta($post_id, 'back_speed', true)) ? esc_attr(get_post_meta($post_id, 'back_speed', true)) : 0; ?>" name="back_speed"/>
                            <br><span class="description">backspacing speed in milliseconds </span>
                        </td>
                    </tr>
                    <tr>
                        <td>Back Delay</td>
                        <td>
                            <input type="text" value="<?php echo esc_attr(get_post_meta($post_id, 'back_delay', true)) ? esc_attr(get_post_meta($post_id, 'back_delay', true)) : 500; ?>" name="back_delay"/>
                            <br><span class="description">time before backspacing in milliseconds </span>
                        </td>
                    </tr>
                    <tr>
                        <td>Loop</td>
                    ...
                    <tr>
                        <td>Cursor Char</td>
                        <td>
                            <input type="text" value="<?php echo esc_attr(get_post_meta($post_id, 'cursor_char', true)) ? esc_attr(get_post_meta($post_id, 'cursor_char', true)) : '|'; ?>" name="cursor_char"/>
            ...
            <table class="widefat">
                <tbody>
                    <tr>
                        ...
                        <td>
                            <input type="text" placeholder="eg. 24" value="<?php echo esc_attr(get_post_meta($post_id, 'font_size', true)); ?>" name="font_size"/>
                        </td>
                    </tr>
                    <tr>
                        <td style="width: 15%;">Font Family</td>
                        <td>
                            <input type="text" placeholder='Helvetica,Arial,sans-serif' value="<?php echo esc_attr(get_post_meta($post_id, 'font_type', true)); ?>" name="font_type"/>
                        </td>
                    </tr>
                    <tr>
                        <td style="width: 15%;">Font Weight</td>
                        <td>
                            <input type="text" placeholder="eg. 300" value="<?php echo esc_attr(get_post_meta($post_id, 'font_weight', true)); ?>" name="font_weight"/>
                        </td>
                    </tr>
                    <tr>
                        <td>Color</td>
                        <td>
                            <input class="typed-color-field" type="text" value="<?php echo esc_attr(get_post_meta($post_id, 'font_color', true)); ?>" name="font_color"/>
                        </td>
                    </tr>
                    <tr>
                        <td>Anchor Color</td>
                        <td>
                            <input class="typed-color-field" type="text" value="<?php echo esc_attr(get_post_meta($post_id, 'anchor_color', true)); ?>" name="anchor_color"/>
                        </td>
                    </tr>
                    <tr>
                        <td>Cursor Color</td>
                        <td>
                            <input class="typed-color-field" type="text" value="<?php echo esc_attr(get_post_meta($post_id, 'cursor_color', true)); ?>" name="cursor_color"/>
                        </td>
                    </tr>                    
                </tbody>
            </table>
        ...
        public function typed_save_meta_box_data($post_id) {
            ...
            if (isset($_POST['post_type']) && 'typed' == $_POST['post_type']) {
                ...
                if (isset($_POST['font_type']))
                    update_post_meta($post_id, 'font_type', sanitize_text_field($_POST['font_type']));

                if (isset($_POST['font_color']))
                    update_post_meta($post_id, 'font_color', sanitize_text_field($_POST['font_color']));

                if (isset($_POST['anchor_color']))
                    update_post_meta($post_id, 'anchor_color', sanitize_text_field($_POST['anchor_color']));

                if (isset($_POST['cursor_color']))
                    update_post_meta($post_id, 'cursor_color', sanitize_text_field($_POST['cursor_color']));

                if (isset($_POST['font_weight']))
                    update_post_meta($post_id, 'font_weight', sanitize_text_field($_POST['font_weight']));
                ...
                if (isset($_POST['type_speed']))
                    update_post_meta($post_id, 'type_speed', sanitize_text_field($_POST['type_speed']));

                if (isset($_POST['start_delay']))
                    update_post_meta($post_id, 'start_delay', sanitize_text_field($_POST['start_delay']));

                if (isset($_POST['back_speed']))
                    update_post_meta($post_id, 'back_speed', sanitize_text_field($_POST['back_speed']));

                if (isset($_POST['back_delay']))
                    update_post_meta($post_id, 'back_delay', sanitize_text_field($_POST['back_delay']));

                if (isset($_POST['typed_loop']))
                    update_post_meta($post_id, 'typed_loop', sanitize_text_field($_POST['typed_loop']));
                else
                    update_post_meta($post_id, 'typed_loop', 'no');

                if (isset($_POST['show_cursor']))
                    update_post_meta($post_id, 'show_cursor', sanitize_text_field($_POST['show_cursor']));
                else
                    update_post_meta($post_id, 'show_cursor', 'no');

                if (isset($_POST['blinking_cursor']))
                    update_post_meta($post_id, 'blinking_cursor', sanitize_text_field($_POST['blinking_cursor']));
                else
                    update_post_meta($post_id, 'blinking_cursor', 'no');

                if (isset($_POST['cursor_char']))
                    update_post_meta($post_id, 'cursor_char', sanitize_text_field($_POST['cursor_char']));
            }
        }
        ...
        public function typed_func($attr) {
            ...
            if (isset($attr['id'])) {
                ...
                if ($typed) :
                    ...
                    <div class="typed-div-<?php echo $post_id; ?>">                        
                        <span style="white-space:pre;" class="typed_perview_div" data-cursor-char="<?php echo esc_attr(get_post_meta($post_id, 'cursor_char', true)); ?>" data-cursor="<?php echo esc_attr(get_post_meta($post_id, 'show_cursor', true)); ?>" data-strings="<?php echo esc_attr($strings); ?>" data-speed="<?php echo esc_attr(get_post_meta($post_id, 'type_speed', true)); ?>" data-delay="<?php echo esc_attr(get_post_meta($post_id, 'start_delay', true)); ?>" data-back-delay="<?php echo esc_attr(get_post_meta($post_id, 'back_delay', true)); ?>" data-back-speed="<?php echo esc_attr(get_post_meta($post_id, 'back_speed', true)); ?>" data-loop="<?php echo esc_attr(get_post_meta($post_id, 'typed_loop', true)); ?>"></span>
                    </div>
                    <?php
                endif;
            }
